...
router.post("/iveri/initiate", async (req, res) => {
  ...
  try {
    ...
    if (paymentContext === "BOOKING") {
      ...
      const billReconciliation = reconcileClientBreakdown({
        context: "BILL_PAYMENT",
        server: { originalAmount, discountAmount, payableAmount: amountMajor },
        client: clientBreakdown,
      });
      if (!billReconciliation.ok) {
        console.warn("[iVeri initiate][BILL_PAYMENT amount mismatch]", billReconciliation.mismatches);
        return res.status(400).json({
          error: "Amount mismatch between app and server. Please refresh and try again.",
          code: "AMOUNT_MISMATCH",
          details: billReconciliation.mismatches,
        });
      }
