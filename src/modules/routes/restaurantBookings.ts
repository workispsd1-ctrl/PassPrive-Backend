...
function normalizeRestaurantId(value: string | { id: string }) {
...
async function getAuthenticatedCustomer(req: any, res: any) {
...
router.post("/confirm", async (req, res) => {
  ...
  try {
    ...
    if (!restaurant || restaurant.is_active !== true) {
      return res.status(404).json({ error: "Restaurant not found", code: "RESTAURANT_NOT_FOUND" });
    }
    if (restaurant.booking_enabled !== true) {
      return res.status(400).json({ error: "Bookings are disabled", code: "BOOKING_DISABLED" });
    }
    if (!Number.isInteger(partySize) || partySize <= 0) {
      return res.status(400).json({ error: "Invalid guest count", code: "INVALID_PARTY_SIZE" });
    }

    const now = new Date();
    const bookingDateTime = new Date(`${bookingDate}T${bookingTime}`);
    if (Number.isNaN(bookingDateTime.getTime())) {
      return res.status(400).json({ error: "Invalid slot", code: "INVALID_SLOT" });
    }
    ...
    if (bookingDateTime > latestBookingDate) {
      return res.status(400).json({
        error: "Selected date exceeds advance booking window",
    ...
    if (openingWindows.length > 0) {
      ...
      if (!isOpenForSlot) {
        return res.status(400).json({
          error: "Selected time is outside restaurant opening hours",
          code: "INVALID_SLOT",
        });
      }
    }
    ...
    if (!isRegularBooking) {
      ...
      if (!verifiedOffer) {
        return res.status(400).json({
          error: "Selected offer is invalid or inactive",
          code: "INVALID_OFFER",
        });
      }
    }
    ...
    const paymentRequired = verifiedCoverChargeAmount > 0;
    const payment = body.payment ?? null;
    const paymentVerified = !paymentRequired || isPaymentVerified(payment);

    if (paymentRequired && !paymentVerified) {
      return res.status(402).json({
        error: "Payment verification is required before confirming this booking",
  ...
  : "paid";
    ...
    const insertPayload = {
      ...
      duration_minutes: parseNumericSignal(restaurant.avg_duration_minutes) || 90,
      ...
      selected_offer: buildSelectedOfferSummary(verifiedOffer),
    ...
    if (insertError) {
      return res.status(500).json({ error: insertError.message, code: "BOOKING_INSERT_FAILED" });
    }
